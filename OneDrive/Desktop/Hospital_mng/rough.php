...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    switch ($option) {
        ...
        case "month":
            ...
            break;
        case "period":
            $sqlLab = "SELECT l.*, p.first_name, p.middle_name, p.last_name FROM lab_reports l JOIN patient p ON l.pa_id = p.pa_id WHERE l.date BETWEEN '$startDate' AND '$endDate'";
    ...
    if ($resultLab->num_rows > 0) {
        echo "<table>";
        echo "<tr><th>Report No</th><th>PA ID</th><th>Patient Name</th><th>Doctor ID</th><th>Date</th><th>Category</th><th>Remarks</th><th>Amount</th></tr>";
        while ($row = $resultLab->fetch_assoc()) {
            $fullName = $row["first_name"] . " " . $row["middle_name"] . " " . $row["last_name"];
            echo "<tr><td>" . $row["report_no"] . "</td><td>" . $row["pa_id"] . "</td><td>" . $fullName . "</td><td>" . $row["doc_id"] . "</td><td>" . $row["date"] . "</td><td>" . $row["category"] . "</td><td>" . $row["Remarks"] . "</td><td>" . $row["amount"] . "</td></tr>";
